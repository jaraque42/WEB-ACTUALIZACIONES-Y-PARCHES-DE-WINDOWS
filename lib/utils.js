 ...
 * @param {string} alias - Alias de la actualización (ej: CVE-2024-21318 o 2024-Apr)
 * @returns {string} URL correcta
 */
export function getMSRCUrl(alias) {
  if (!alias) return '#'
  
  // Si es un CVE, usar el path de vulnerability
  if (/^CVE-\d{4}-\d+$/i.test(alias)) {
    return `https://msrc.microsoft.com/update-guide/vulnerability/${alias}`
  }
  
  // Si parece una fecha de release (ej: 2024-Apr), usar releaseNote
  if (/^\d{4}-[A-Z][a-z]{2}$/.test(alias)) {
    return `https://msrc.microsoft.com/update-guide/releaseNote/${alias}`
  }
  
  // Default a vulnerability por si acaso
  return `https://msrc.microsoft.com/update-guide/vulnerability/${alias}`
}
